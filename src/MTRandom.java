 ...
 * generator algorithm
 * based upon the original C code by Makoto Matsumoto and Takuji Nishimura (see
 * <a
 * href="http://www.math.sci.hiroshima-u.ac.jp/m-mat/MT/MT2002/emt19937ar.html">
 * http://www.math.sci.hiroshima-u.ac.jp/m-mat/MT/MT2002/emt19937ar.html</a> for
 * more information).
 *
 * <p>
 * As a subclass of {@link java.util.Random}, this class provides a single
 * canonical {@link
 * Random#next(int)} method for generating bits in the pseudo random number
 * sequence. Anyone using
 * this class should invoke the public inherited methods
 * ({@link Random#nextInt()}, {@link
 * Random#nextFloat()} etc.) to obtain values as normal. This class should
 * provide a drop-in
 * replacement for the standard implementation of {@code java.util.Random} with
 * the additional
 * advantage of having a far longer period and the ability to use a far larger
 * seed value.
 *
 * <p>
 * This is <b>not</b> a cryptographically strong source of randomness and should
 * <b>not</b> be
 * used for cryptographic systems or in any other situation where true random
...
public final class MTRandom extends Random {
     ...
     * serialisation of its
     * internal state, and it may even be necessary to implement read/write methods
     * to re-seed it
    ...
    // Constants used in the original C implementation. The names here match the
    // code in:
    ...
    // http://www.math.sci.hiroshima-u.ac.jp/m-mat/MT/MT2002/CODES/mt19937ar.c
    //
    // State vector (length N). This cannot be final due to how java.util.Random is
    // initialized.
    private int[] mt;
    // State vector index (0 <= mti <= N). Note that this *can* obtain the value N
    // temporarily, and
    // indicates that the state vector must be re-processed before the next random
    // value can be taken.
    private int mti;

    // Compatibility mode - used to mimic original C-code exactly (though this is
    // not recommended).
    //
    // WARNING: This is read before it is initialized due to java.util.Random
    // invoking setSeed(long)
    // in the parent constructor. We detect this by checking for {@code null} in our
    // version of
    // setSeed(long), and ignore those calls. We always set a seed value manually
    // later.
    private final Mode compatibilityMode;

    // Private constructor which must be called by all other constructors to
    // indicate subclass
    ...
    /**
     * Creates a new {@code MTRandom} instance. This constructor sets the seed of
     * the random number
     * generator to a value very likely to be distinct from any other invocation of
     * this constructor.
     *
     * <p>
     * Note that the method to generate a default seed is explicitly not defined and
     * may change
     * without notice as improvements are made. If you need to rely on deterministic
     * output, you
     ...
     * to the original C
     * code version of this algorithm including exactly replicating the case where
     * the seed value had
     * not been set prior to calling {@code genrand_int32}.
     *
     * <p>
     * If {@code compatible} is set to {@code true}, then the algorithm will be
     * seeded with the
     * same default value as was used in the original C code. Furthermore, the
     * {@link
     * Random#setSeed(long)} method, which must take a 64 bit {@code long} value,
     * will be limited to
     ...
     *
     * <p>
     * While useful for ensuring backwards compatibility, it is advised that this
     * feature not be
     * used unless specifically required, due to the reduction in strength of the
     * seed value.
     *
     * <p>
     * If {@code compatible} is set to {@code false}, this constructor behaves
     * exactly the same as
     * {@link #MTRandom()}.
     *
     ...
     * 64 bit seed value.
     * For a better random number sequence this seed value should contain as much
     * entropy as possible.
     *
    ...
    // required as part of the Mersenne Twister algorithm but need not be made
    // public.
    //
    // Note that every element in the state vector is written sequentially, and
    // later
    ...
    private void initStateVector(int seed) {
        // Annoying runtime check for initialisation of internal data caused by
        // java.util.Random
        // invoking setSeed() during init. This is unavoidable because no fields in our
        // instance will
        // have been initialised at this point, not even if the code were placed at the
        // declaration of
        // the member variable.
        if (mt == null)
            mt = new int[N];
     ...
     * provided. Note
     * that if the same seed data is passed to two different instances of MTRandom
     * (both of which
     * share the same compatibility state) then the sequence of numbers generated by
     * both instances
     * will be identical.
     *
     * <p>
     ...
     * will only use the
     * lower 32 bits of any seed value passed in and will match the behaviour of the
     * original C code
    ...
    public synchronized void setSeed(long seed) {
        if (compatibilityMode == null) {
            // This call came from the parent class constructor and should be ignored since
            // we will be
     ...
     * data provided. Note
     * that calling this method is equivalent to calling
     * {@code setSeed(pack(seedBytes))} and in
     * particular will result in a new integer array being generated during the
     * call. If you wish to
     * retain this seed data to allow the pseudo random sequence to be restarted
     * then it would be more
     * efficient to use the {@link #pack(byte[])} method to convert it into an
     * integer array first,
    ...
    public synchronized void setSeed(int[] seedArray) {
        ...
        if (length == 0)
            throw new IllegalArgumentException("Seed buffer cannot not be empty");
        // ---- Begin Mersenne Twister Algorithm ----
     ...
     * from this class.
     * If given a value of 32, this method behaves identically to the genrand_int32
     * function in the
     * original C code and ensures that using the standard nextInt() function
    ...
    protected synchronized int next(int bits) {
        ...
        if (mti >= N) { // generate N words at one time
            ...
            // In the original C implementation, mti is checked here to determine if
            // initialisation has
            // occurred; if not it initialises this instance with DEFAULT_SEED (5489). This
            // is no longer
            // necessary as initialisation of the Java instance must result in
            // initialisation occurring
     ...
     *
     * <p>
     * This simple utility method can be used in cases where a byte array of seed
     * data is to be
     * used to repeatedly re-seed the random number sequence. Packing the byte array
     * into an integer
     * array first using this method, and then invoking {@link #setSeed(int[])} with
     * that, removes the
     ...
     * will produce an
     * empty integer array, but the {@code setSeed()} method will throw an exception
     * if the empty
    ...
    // Internal method to generate a "probably unique" seed array with a
    // "reasonable" number of bits
    ...
    // and listed as a good accumulator for a 64-bit modulus (when no constant
    // adjustment is used).
    ...
    private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
        ...
        // Since you should not be able to serialize the instance until it's
        // constructed, we expect
        // internal invariants to be maintained.
        //
        // Note that while "mti == mt.length" is possible during nextInt(), that method
        // is synchronized
